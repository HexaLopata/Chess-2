...
public class RandomTeleport : Skill
{
    ...
    private IEnumerator MoveAndUpdateTurns(BattleFieldFigure figure, BattleFieldCell cell)
    {
        ...
        if(figure.GetRelevantMoves(figure.BattleField.BattleFieldCells).Length > 0)
            figure.BattleField.ActivateAllCells(figure.GetRelevantMoves(figure.BattleField.BattleFieldCells));
        else
